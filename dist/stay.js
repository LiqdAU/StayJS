...
window.Stay = (function ($) {
  class Stay {
    ...
    reset(opts) {
      opts = { ...this.options, ...opts }; // Defaults
      ...
      opts.isReady =
        typeof opts.isReady !== "function"
          ? this.store.fn.isReady
          : opts.isReady;
      opts.overlayScrollbars = opts.overlayScrollbars !== false;
      opts.hashOpts = opts.hashOpts || {};
      this.sections = [];
      this.store.elements.scroller = $(
        opts.scroller || document.documentElement
      ); // Remove window clones

      $("." + this.classes.clone.window).remove();
      this.store.elements.windowClone = null;
      this.store.fn.isReady = opts.isReady; // Add classes to wrap/scrollwrap
    ...
    updateDebug(vars) {
      if (this.store.elements.debug) {
        Object.entries(vars).forEach((i) => {
          this.store.elements.debug
            .find(`[data-debug="${i[0]}"] span`)
            .text(i[1]);
        });
      }
    }
    ...
    triggerChange(section, forward, index) {
      ...
      $(this.store.elements.scroller).trigger("stayChange", [
        section,
        index,
        forward
      ]);
    ...
    refresh() {
      ...
      if (
        ...
        typeof this.store.page.current.before === "function"
      ) {
    ...
    setNavigation(opts) {
      ...
      if (opts.hasOwnProperty("enabled")) {
        if (opts.enabled) {
          let wrap = $("<nav>", {
              class: this.classes.nav.wrap
            }),
            back = $("<a>", {
              href: "#",
              class: this.classes.nav.back
            }),
            next = $("<a>", {
              href: "#",
              class: this.classes.nav.next
            });
      ...
      if (
        opts.hasOwnProperty("update") &&
        opts.update === true &&
        this.store.nav
      ) {
        let show =
            this.store.page.current &&
            this.store.page.current.showNav !== false,
        ...
        if (show && prev && prev.showInNav !== false) {
          ...
          this.store.nav.prevTimeout = delay
            ? setTimeout(unhide, delay)
            : unhide();
        } else {
          this.store.nav.back.text("").addClass(this.classes.hidden);
        }
        ...
        if (show && next && next.showInNav !== false) {
          ...
          this.store.nav.nextTimeout = delay
            ? setTimeout(unhide, delay)
            : unhide();
        } else {
          this.store.nav.next.text("").addClass(this.classes.hidden);
        }
      }
    }
    ...
    setAbsolute(absolute) {
      ...
      if (absolute) {
        ...
        $("." + this.classes.section.class + ":visible").css(
          "transform",
    ...
    setActive(name) {
      let active =
        typeof name === "string"
          ? this.sections.find((s) => s.name === name)
          : name;

      if (active) {
        $("." + this.classes.section.active).removeClass(
          this.classes.section.active
        );
    ...
    scrollTo(to, args) {
      ...
      args.duration = duration
        ? duration
        : args.duration || this.options.navigation.speed || 1000;
    ...
    add(section) {
      ...
      if (!this.store.elements.windowClone) {
        this.store.elements.windowClone = this.addClone(
          "100vh",
          false,
          this.classes.clone.window
        );
    ...
    static resolvePoint(deg, length, start) {
      ...
      return {
        x: x + ((start === null || start === void 0 ? void 0 : start.x) || 0),
        y: y + ((start === null || start === void 0 ? void 0 : start.y) || 0)
      };
    }
